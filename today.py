...
try: # File may currently hold 40 character token only.
    with open('/home/anil/.config/todoist.conf', 'r') as cache: token = cache.read()
    ##raise Exception(100, 'Failed to find auth token')
except Exception as e:
    print('{0}{1}: {2} [AUTH]'.format(prefix4, e.args[-1], e.args[0]))
    print('{0}{1}: {2} [AUTH]'.format(prefix4, 'Failed to find auth token', 'FATAL'))
...
try: # Try to read the project list from cache first to save a network request.
    with open('/home/anil/.config/conky/today.projects.json', 'r') as cache:
    ...
    try: # If cache not available retrieve project list and then cache it.
        ...
        with request.urlopen('?'.join((projects_url, dat))) as res:
            ...
            with open('/home/anil/.config/conky/today.projects.json', 'w') as cache:
...
try: # Always try the server first to get an up to date list.
    ...
    dat.sort(key=lambda t: projects.get(t['project_id'], '')) # Stable secondary sort key project name.
    dat.sort(key=lambda t: t['priority'], reverse=True) # Primary sort on priority.
    for task in dat:
...
# TODO: Truncate lines to 50 or so chars with trailing ...
# TODO: Support double line task names when long enough.
# TODO: Cache task list and display in grey when off line.
